...
export default function CreateUniversitas() {
  const navigate = useNavigate();
  const addUniversity = useUniversitasStore((s) => s.addUniversity);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError(null);
    try {
      await addUniversity({ name: name.trim(), email: email.trim(), address: address.trim() });
      navigate("/dashboard/universitas");
    } catch (err: any) {
      setError(err.message || "Gagal menambahkan universitas.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="flex flex-col gap-8 font-sans w-full text-slate-700 select-none">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-xs text-slate-400 font-medium">
          Isi formulir di bawah untuk mendaftarkan kampus mitra baru ke dalam sistem.
        </p>
      </div>
        ...
        <div className="p-6 sm:p-8 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <span className="text-[10px] text-slate-400">
              Slug akan dibuat otomatis oleh sistem dari nama ini.
            </span>
          </div>
