...
export default function CreateBidang() {
  ...
  const addBidang = useBidangStore((s) => s.addBidang);
  const role = useAuthStore((s) => s.user?.role);

  const [name, setName] = useState("");
  const [submitting, setSubmitting] = useState(false);

  // Hanya Super Admin yang boleh menambah bidang (bidang bersifat global lintas kampus).
  if (role !== "super_admin") {
  ...
  return (
    <div className="flex flex-col gap-8 font-sans w-full text-slate-700 select-none">
      <div className="flex flex-col gap-2">
        ...
        <h2 className="text-xl font-bold text-slate-800 tracking-tight">Tambah Bidang Baru</h2>
        <p className="text-xs text-slate-400 font-medium">Isi formulir di bawah untuk menambahkan bidang/industri baru.</p>
      </div>
